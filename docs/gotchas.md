...
`<公司名>H股公告-2024年年度报告`,标题也以「2024年年度报告」结尾,且**在返回
列表里排在 A 股版前面** —— 不排除就会把港式合并年报(繁体、无证监会标准章节)
当正文抓下来。建设银行、中国银行都栽在这上面。

"排在前面"**不等于"晚发"**:建行 H 股版 2025-04-26 / A 股版 2025-03-29(晚 28 天),
而中行两份**同为 2025-03-27** 却仍排在前面 —— 同日内的次序另有其因。别按"晚发
一个月"去理解这件事,更**别按日期过滤**(同日的过滤不掉)。

**注意这个问题只在按单股查询时可见**:`stock=<6位>,<orgId>` 的路径能拿到这份
H 股公告,而**全市场按 `seDate`+`column` 枚举的路径里一条都没有**(FY2024 全量
5,712 条正文记录里,标题含「H股」的 0 条)。所以拿 `query_all` 去复现会得出
"H 股排除是多余的"这个错误结论 —— 要验就得按单股查(建行 601939 / 中行 601988
各 3 条,一眼可见)。
