...
# 年度报告有三种写法:「2024年年度报告」(多数) / 「2024年度报告」(四大行、招行) /
# 「2024年年报报告」(紫金矿业 —— 发行人自己写重了,不是披露类型差异)。
_REPORT_TAIL_RE = re.compile(
    r"\d{4}年(?:年?度|年报)报告$|\d{4}年(?:第一季度|半年度|第三季度)报告$"
)
# H股公告：A+H 公司会另挂一份「XXH股公告-2024年年度报告」，标题同样以
# 「2024年年度报告」结尾，且**排在 A 股版前面** —— 不排除就会把港式合并年报
# (繁体、无证监会标准章节)当正文。建设银行、中国银行都栽在这上面。
#
# "排在前面"不等于"晚发"：建行 H 股版 2025-04-26 / A 股版 2025-03-29，中行两份
# 同为 2025-03-27 却仍排在前面 —— 别按日期过滤，同日的过滤不掉。
# 另：这份公告只在按单股查询(stock=6位,orgId)时可见，全市场按 seDate+column
# 枚举的路径里一条都没有 —— 拿全市场翻页去复现会误判成"这条排除是多余的"。
