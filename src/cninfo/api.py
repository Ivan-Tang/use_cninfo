...
# 「2024年年度报告」结尾,会被 is_kind_report_body 误收。它**排在 A 股版前面**,
# 于是 find_periodic_report 抓到的是港式合并年报(繁体、无「报告期内公司从事的
# 主要业务」章节),而不是 A 股正文。建设银行、中国银行都栽在这上面。
#
# 「排在前面」不等于「晚发」:建行 H 股版 2025-04-26 / A 股版 2025-03-29,中行
# 两份同为 2025-03-27 却仍排在前面 —— 所以别按日期过滤,同日的过滤不掉。
# 另:这份 H 股公告只在按单股查询(stock=6位,orgId)时可见,全市场按 seDate+column
# 枚举的路径里一条都没有(实测 FY2024 全量 5,712 条正文记录,含「H股」的 0 条)——
# 拿 query_all 去复现会误判成"H 股排除是多余的"。
